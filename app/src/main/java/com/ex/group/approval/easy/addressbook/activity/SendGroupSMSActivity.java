...
//        	params.put("infoCompanyCd", comCd);
	...
	public void onPostThread(XMLData a_XMLData, SKTException a_exception) {
		...
		isSelectAll = false;
		mBtnSelect.setText(R.string.easyaproval_select_all);
...
//					}
	...
	public void onClick(View v) {
		...
		if(v.getId() == R.id.multiselecttitle_check) {
	    	...
	    	if(isSelectAll) {
	    		mTempButton.setText(R.string.easyaproval_unselect_all);
	    	} else {
	    		mTempButton.setText(R.string.easyaproval_select_all);
	    	}
			...
			for(int i = 0; i < data.size(); i++) {
				...
				if(sSmsList.size() >= 50) {
					m_szTitle = getString(R.string.easyaproval_add_receiver);
		    		m_szDialogMessage = getString(R.string.easyaproval_add_receiver_text);
					showDialog(DIALOG_MEMBER_LIST);
					break;
				}
...
//			startActivityForResult(intent, 1);
	...
	private void updateReceiveList() {
		mAddList.setText(getString(R.string.easyaproval_receive_list) + " (" + sSmsList.size() + "/50)");
		mAddList2.setText(getString(R.string.easyaproval_receive_list) + " (" + sSmsList.size() + "/50)");
	}
...
//    	mUpButtonText.setText(a_szTeamName);
    	...
    	mUpButton.setBackgroundResource(R.drawable.easy_member_btn_updept_selector);
...
//    	mLogoLayout.setVisibility(View.VISIBLE);
    	...
    	mUpButton.setBackgroundResource(R.drawable.easy_member_btn_updept_off_selector);
    ...
    public BaseAdapter mTeamAdapter = new BaseAdapter() {
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if(data.isMember) {
            	...
            	convertView = getLayoutInflater().inflate(R.layout.easy_member_employeeteam, null);
...
//            	dv.setText(data.m_szDepartment);
                ...
                if(data.m_bIsChecked) {
	        		iv.setImageResource(R.drawable.easy_member_btn_check);
	        	} else {
	        		iv.setImageResource(R.drawable.easy_member_btn_uncheck);
	        	}
    ...
    public OnItemClickListener mTeamClickListener = new OnItemClickListener() {
		public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
            ...
            if(data.isMember) {
				...
				if(data.m_bIsChecked) {
        			data.m_bIsChecked = false;
        			phone.setImageResource(R.drawable.easy_member_btn_uncheck);
        		} else {
        			data.m_bIsChecked = true;
        			phone.setImageResource(R.drawable.easy_member_btn_check);
        		}
